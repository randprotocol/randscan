...
|---|---|---|---|
| Rand | 1 | this fullnode | `randprotocol-core::bridge` |
...
| `randprotocol-core::bridge` | fullnode `crates/randprotocol-core/src/bridge` | `verify` (signatures), `BridgeState` (state and root), `check_attest` / `check_burn` / `apply_*` |
| `Ledger` | fullnode `crates/randprotocol-core/src/ledger.rs` | admission order for both kinds, fee debit, state root composition |
| Storage | fullnode `crates/randprotocol-node/src/storage.rs` | three RocksDB column families plus a meta blob; replay and integrity check |
| JSON-RPC | fullnode `crates/randprotocol-node/src/rpc.rs` | submission plus five bridge query methods |
| Wallet | fullnode `crates/randprotocol-client` | `bridge-mint`, `bridge-burn`, `asset-balance`, `bridge-status` |
...
`crates/randprotocol-core/src/bridge/vectors.json` in the fullnode holds 39 vectors generated by the
...
Today, none. Neither `bridge-codec` nor `randprotocol-core::bridge` imports anything from the zkVM or
...
- fullnode `crates/randprotocol-core/src/bridge/{mod,state}.rs`, `src/ledger.rs`, `src/gas.rs`
- fullnode `crates/randprotocol-node/src/{rpc,storage}.rs`
